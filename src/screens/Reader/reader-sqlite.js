...
function Reader({ navigation, route,goBack }) {
    ...
    const createTables = () => {
        db.transaction(txn => {
          txn.executeSql(
            `CREATE TABLE IF NOT EXISTS locations (id INTEGER PRIMARY KEY AUTOINCREMENT, location json,idbook VARCHAR(30))`,
      ...
      const addLocation = (idbook,location) => {
        ...
        db.transaction(txn => {
          ...
          txn.executeSql(
            `SELECT * FROM locations where idbook=? LIMIT 1`,
            [idbook],
            (sqlTxn, res) => {
              ...
              if (len > 0) {
                ...
                txn.executeSql(
                    `INSERT INTO locations (location,idbook) VALUES (?,?)`,
                    [location, idbook],
                    (sqlTxn, res) => {
    ...
    async function _getLocation(locationDB) {
          try {
             let themebook=null;
             const value = await AsyncStorage.getItem('location'+idbook);
             const theme = await AsyncStorage.getItem('theme');
             //alert("theme"+theme+"loc = "+value);
...
//webview.current?.injectJavaScript(`window.rendition.display("epubcfi(/6/4[item5]!/4/24/34/8/4/4/6/32/20/44/4/50/1:50)")`);
            ...
            if (value == null) {
                ...
                //console.log("----val = null",locations);
                console.log("----val = null",locationDB);
                setJs(`window.BOOK_PATH = "${path}";window.BOOK_LOCATION = "${value}";window.LOCATIONS = ${locationDB};window.THEME = ${JSON.stringify(themeToStyles(themebook))};`);
             
                setLastLoc(0);
            }else{
                console.log("----val = not---- null",locationDB);
    ...
    useEffect(() => {
        //console.log("---k--",typeof(locations),"value",locations);
    ...
    function handleMessage(msg) {
        ...
        switch (type) {
            ...
            case 'isStart':
                ...
                return;
            case 'locations':
                //addLocation(idbook,parsedData.locations);
                if (isLoading) {
